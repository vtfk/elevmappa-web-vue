const getAuthConfig = (url = 'http://localhost:8080') => ({
  auth: {
    authority: process.env.VUE_APP_AUTH_AUTHORITY || 'https://login.microsoftonline.com/your-tenant-id/v2.0',
    clientId: process.env.VUE_APP_AUTH_CLIENT_ID || 'your-client-id',
    redirectUri: process.env.VUE_APP_AUTH_REDIRECT_URI || `${url}/oidc-callback`,
    responseType: process.env.VUE_APP_AUTH_RESPONSE_TYPE || 'id_token token',
    scope: process.env.VUE_APP_AUTH_SCOPE || 'openid profile email',
    automaticSilentRenew: process.env.VUE_APP_AUTH_SILENT_RENEW || false,
    silentRedirectUri: process.env.VUE_APP_AUTH_SILENT_URI || `${url}/silent-renew-oidc.html`,
    prompt: process.env.VUE_APP_AUTH_PROMPT || 'login'
  }
})

export default {
  apiUrl: process.env.VUE_APP_ELEVMAPPA_API_URL || 'https://api.elevmappa.no/api',
  apiTimeout: process.env.VUE_APP_ELEVMAPPA_API_TIMEOUT || 10, // Timeout in seconds
  company: {
    name: process.env.VUE_APP_ELEVMAPPA_COMPANY_NAME || 'Vestfold og Telemark',
    logo: process.env.VUE_APP_ELEVMAPPA_COMPANY_LOGO || 'data:image/png;base64,',
    supportMail: process.env.VUE_APP_COMPANY_SUPPORT_MAIL || 'post@vtfk.no'
  },
  productOwner: process.env.VUE_APP_PRODUCT_OWNER || 'Vestfold og Telemark',
  fonts: {
    primary: process.env.VUE_APP_FONT_PRIMARY || 'Nunito',
    title: process.env.VUE_APP_FONT_TITLE || 'Nunito Sans'
  },
  colors: {
    primary: process.env.VUE_APP_ELEVMAPPA_COLOR_PRIMARY || '#005260',
    background: process.env.VUE_APP_ELEVMAPPA_COLOR_BACKGROUND || '#f7f7f7',
    secondary: process.env.VUE_APP_ELEVMAPPA_COLOR_SECONDARY || '#ebe6d6',
    accent: process.env.VUE_APP_ELEVMAPPA_COLOR_ACCENT || '#6ac4ae',
    error: process.env.VUE_APP_ELEVMAPPA_COLOR_ERROR || '#e40017',
    info: process.env.VUE_APP_ELEVMAPPA_COLOR_INFO || '#00b1c7',
    success: process.env.VUE_APP_ELEVMAPPA_COLOR_SUCCESS || '#C0DCB2',
    warning: process.env.VUE_APP_ELEVMAPPA_COLOR_WARNING || '#FFCD00'
  },
  ...getAuthConfig(process.env.VUE_APP_ELEVMAPPA_PROD_URL)
}
